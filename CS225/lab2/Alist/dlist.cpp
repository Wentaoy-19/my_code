...
template<class T> void DList<T>::bubble_dlist()
{
    int i,j;
    T temp;
...
template<class T> void DList<T>::insertion_sort()
{
    int low,high,mid;
    int i;
    T current_element;
    ...
    {
        current_element=getitem(i);
        low=0;
        high=i-1;  
        while(low<=high)
        {
            mid=(high+low)/2;
            if(current_element>=getitem(mid))
            {
                high=mid-1;
            }
            else
